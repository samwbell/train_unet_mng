...
true_loc_file_list = sorted(glob.glob('handlocs/1898*.txt'), key=naturalSort)
...
for true_loc_file in true_loc_file_list:
	...
	true_locs = true_locs.assign(X=(true_locs['X (um)']/0.2508 - 1000.0).astype(int))
	true_locs = true_locs.assign(Y=(true_locs['Y (um)']/0.2508 + 520).astype(int))

	#fimage = cv2.imread('images/30159.png')

	file_list = sorted(glob.glob('filtered/' + image_num + '*.png'))
	...
	for file in file_list:
		...
		for row in testis_locs.itertuples():
			...
			cv2.circle(masked, (row.x,row.y), 20, (0,255,0), -1)

			#print(row.x,row.y)
			if loc_matchQ(row.x,row.y):
				cv2.circle(filtered, (row.x,row.y), 30, (0,255,0), 5)
				cv2.circle(smalldot, (row.x,row.y), 7, (0,255,0), -1)
				matches += 1
			else:
				cv2.circle(filtered, (row.x,row.y), 30, (0,0,255), 5)
		...
		cv2.imwrite('labeled/' + base + '.labeled.png', filtered)

		cv2.imwrite('masklabeled/' + base + '.masked.labeled.png', masked)

		cv2.imwrite('smalldot/' + base + '.masked.smalldot.png', smalldot)
	...
	fulloutfile += 'Total for ' + image_num + ':\nMatches = ' + str(matches) + '\nFalse positives = ' + str(false_positives) + \
				'\nFalse negatives = ' + str(false_negatives) + '\nF1 score = ' + str(round(f1,3))
	write_file('results/' + image_num + '.results.txt', fulloutfile)
	print(fulloutfile)
...
write_file('results/final.results.txt', finaloutfile)
print(finaloutfile)
